...
public class Lion extends Actor
{
   ...
   { 
      ...
      if(hunt){ //if boolean hunt is true, the startStaring class is called
            startStaring(false);
        }
    }
   ...
   public void setImage() 
    { 
     ...
     else{ //if the lion spawns on the right, this else statement is executed
       if ( position < 190) 
       {    //when the variable position is less than 190 (walking to the right if spawned on the left or to the left
        currentImage++;         // if spawned on the right) the object switches between the images LionRight(1,2,3).fw.png
        if (currentImage == 4) 
   ...
   public void startStaring(boolean first) //when first is true (in walk method) the cat stops walking and this method is executed
   {
      ...
      if(timeJumpCalled + 1000 <= System.currentTimeMillis()){ //after 1/3 second (300 milliseconds), stare boolean becomes false and startStaring 
   ...
   public void setGameOverImage()
   {
      ...
      setLocation (640, 385); //located to the middle of the screen
        if(timeJumpCalled + 300 <= System.currentTimeMillis()){ //after 1/3 scecond, the staring ceases and attcked is called from playscreen
        //hunt = false;
